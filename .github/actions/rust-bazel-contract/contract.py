...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        (workspace / "cargo-bazel-lock.json").write_text("{}\n", encoding="utf-8")
        subprocess.run(
            [
                "git",
                "add",
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        fake_bazelisk.write_text(
            ...
            'printf \'repin=%s\\n\' "${CARGO_BAZEL_REPIN-unset}" >> "$RECORD"\n'
            'printf \'legacy_repin=%s\\n\' "${REPIN-unset}" >> "$RECORD"\n'
            'printf \'generator_url=%s\\n\' "${CARGO_BAZEL_GENERATOR_URL-unset}" >> "$RECORD"\n'
            'printf \'generator_sha=%s\\n\' "${CARGO_BAZEL_GENERATOR_SHA256-unset}" >> "$RECORD"\n'
            'printf \'repin_only=%s\\n\' "${CARGO_BAZEL_REPIN_ONLY-unset}" >> "$RECORD"\n'
            'printf \'isolated=%s\\n\' "${CARGO_BAZEL_ISOLATED-unset}" >> "$RECORD"\n'
            'printf \'debug=%s\\n\' "${CARGO_BAZEL_DEBUG-unset}" >> "$RECORD"\n'
        ...
        driver_env = {
            **os.environ,
            ...
            "CARGO_BAZEL_REPIN": "1",
            "REPIN": "1",
            "CARGO_BAZEL_GENERATOR_URL": "https://evil.invalid/cargo-bazel",
            "CARGO_BAZEL_GENERATOR_SHA256": "0" * 64,
            "CARGO_BAZEL_REPIN_ONLY": "evil-crate",
