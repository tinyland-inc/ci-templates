...
def check_gf_i09_publisher_contract() -> int:
    ...
    # Reread, never relay. The caller-declared path is bound to the actual
    # caller workflow ref, not merely echoed into the receipt.
    ...
    if (
        ...
        not in executable
        or 'test "${GITHUB_WORKFLOW_REF}" = "${expected_workflow_ref}"' not in executable
    ):
        fail("the declared caller workflow path must equal the actual workflow ref")
